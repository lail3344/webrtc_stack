...
typedef struct
{
  ...
  CaptureObserver* captureCallback;
                            ...
                            public ViEEncoderObserver
{
                              ...
                              const unsigned int bitrate)
    {
        ...
        _vieData.codec->GetReceiveCodecStastistics(videoChannel, key_frames,
                                                    delta_frames);

        __android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                            "ReceiveCodecStat, key_frames %d delta_frames %d\n", key_frames, delta_frames);
                              ...
                              const unsigned int bitrate)
    {
        ...
        _vieData.codec->GetSendCodecStastistics(videoChannel, key_frames, delta_frames);
        __android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                            "SendCodecStat, key_frames %d delta_frames %d\n", key_frames, delta_frames);
    }
    ;
 ...
 public:
  ...
  virtual void BrightnessAlarm(const int capture_id,
    ...
    switch (brightness) {
      case webrtc::Normal:
	  	__android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                " BrightnessAlarm Normal");
        
        break;
      case webrtc::Bright:
		__android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                " BrightnessAlarm Bright");
        break;
      case webrtc::Dark:
		__android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                " BrightnessAlarm Dark");
        break;
    }
  }
  ...
  virtual void CapturedFrameRate(const int capture_id,
	...
	__android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                " CapturedFrameRate %d", frame_rate);
  ...
  virtual void NoPictureAlarm(const int capture_id,
    ...
    if (alarm == webrtc::AlarmRaised) {
	  __android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                " NoPictureAlarm CARaised.");
    } else {
	  __android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                "NoPictureAlarm CACleared.");
    }
  }
    ...
    if (vieData.vie) {
      ...
      __android_log_print(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
        ...
        if (enableTrace)
        {
            ...
            __android_log_write(ANDROID_LOG_DEBUG, WEBRTC_LOG_TAG,
                                "SetTraceFilter");
            if (0 != vieData.vie->SetTraceFilter(webrtc::kTraceAll))
            {
    ...
    jint cameraId)
{
  ...
  // added by luqiang
  if (vieData.captureCallback == NULL) {
	  vieData.captureCallback = new CaptureObserver();
  } else {
    vieData.capture->DeregisterObserver(cameraId);
  }
  
  if (vieData.capture != NULL) {
    vieData.capture->RegisterObserver(cameraId, *vieData.captureCallback);	
  }
