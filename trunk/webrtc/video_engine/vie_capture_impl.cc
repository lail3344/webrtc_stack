...
int ViECaptureImpl::StartCapture(const int capture_id,
  ...
  WEBRTC_TRACE(kTraceWarning, kTraceVideo,
	       ViEId(shared_data_->instance_id(), capture_id),
	       "luqiang, Start to capture,  %s: Capture device %d Start to capturing, TickTime::MillisecondTimestamp=%u", 
               __FUNCTION__, capture_id, MaskWord64ToUWord32(TickTime::MillisecondTimestamp()));
