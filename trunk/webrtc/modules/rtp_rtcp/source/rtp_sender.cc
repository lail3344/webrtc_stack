...
int32_t RTPSender::SendOutgoingData(
  ...
  if (audio_configured_) {
    ...
    WEBRTC_TRACE(webrtc::kTraceStream, webrtc::kTraceVideo, id_, 
		 "luqiang,encoded -> RTP sender,  %s: Sending Data, capture_timestamp=%u, capture_time_ms=%u, TickTime::MillisecondTimestamp=%u", 
                 __FUNCTION__, capture_timestamp, MaskWord64ToUWord32(capture_time_ms), MaskWord64ToUWord32(TickTime::MillisecondTimestamp())
                );
