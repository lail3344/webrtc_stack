...
int32_t IncomingVideoStream::RenderFrame(const uint32_t stream_id,
  ...
  // added by luqiang
  WEBRTC_TRACE(kTraceStream, kTraceVideoRenderer, module_id_,
	       "luqiang, start to render, %s for stream %d, render time:%u, framerate: %u, TickTime::MillisecondTimestamp=%u", __FUNCTION__, stream_id_,
	       MaskWord64ToUWord32(video_frame.render_time_ms()), incoming_rate_, MaskWord64ToUWord32(TickTime::MillisecondTimestamp())
              );
